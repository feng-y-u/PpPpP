...
### 33.4 S18 之后的四个跟进项（S19 / S20 / S21 / S22）

这四条是**收口阶段之后**才处理完的：S19 来自本文档之外的分析文档发现（`docs/technical-documentation.md` §20.2），S20 是 §16 那条"状态 API 主动告警"建议的落地补完，S21 是 S19 自己写下的遗留项（Cookie 写入非原子），S22 是 S20 自己写下的遗留项（自动关注没有 `last_error`）。四者都不改既有语义分支，改动范围分别在"一个路径来源"、"一个响应字段 + 一行界面文案"、"一个写盘原语"、"一个运行态字段 + 一行界面文案"。

1. **S19 设置页 Cookie 落点与读取路径不一致**（`routes_settings.api_settings_post`）。写侧按 `__file__` 推项目根，读侧用 `config.COOKIE_PATH`（Linux 上存在 `/etc/pixiv-viewer/cookies.txt` 时优先它）。那种部署里设置页写的是**没人读的文件**：进程内靠直接赋值 `fetcher._cookie_value` 看着生效、重启后旧 Cookie 复辟；且 `get_pooled_session()` 的失效戳盯的也是 `COOKIE_PATH`，那个 mtime 从未变过，新 Cookie **连当期都不对已缓存的连接池生效**（后一点原审计未写出来）。修法：落点改用 `app.COOKIE_PATH`（`config.COOKIE_PATH` 的再导出），写失败时 500 并把实际路径写进错误信息（此前会静默写进一个无害文件然后假装成功）。**证伪**：改回旧的 `__file__` 推导后 `tests/test_settings_api.py` 4 例失败（新用例在 `COOKIE_PATH` 处 `FileNotFoundError`、写失败用例变成 `assert 200 == 500`、夹具的"仓库根真实 cookies.txt 未被改写"断言触发）。**副产物**：`test_fetcher.py` 有 3 例走真实 `_fetch_details_parallel → build_pixiv_session() → _load_cookie()`，此前默默依赖开发者本机存在真实 `cookies.txt`（干净 checkout 上会 `FileNotFoundError`），已改用临时文件夹具；`_isolate_cookies_txt` 的收尾也从"只断言"改为"先无条件还原再断言"（旧版在一次证伪跑动中真的把仓库根的真实 Cookie 覆盖成了测试 token —— 该文件在 `.gitignore` 里，没有任何副本可恢复）。**遗留**：Cookie 写入仍非原子（`open(...,'w')`，理论上可读到半截内容后按 mtime 缓存住这个坏值）—— 该遗留已在 **S21** 处理（见下）。
2. **S20 自动关注"静默停止"在界面上看不见**（§16 的"状态 API 主动告警"只做了一半）。S12 给 `background.get_background_health()` 加了 `auto_follow_alive`，但它只出现在 `/api/prefetch/status`，而设置页既不读那个键、也不读 `/api/auto-follow/status`（该路由返回 `_auto_follow_state`，**同样没有任何前端调用者**）—— 于是"自动关注线程死了"或"每轮都在失败"这两种静默失效，只能在日志里发现。修法：`/api/auto-follow/status` 增补 `alive`（取自 `get_background_health()`，与预取状态页里的同名字段**同源**，只是视角不同），设置页「自动关注」卡片新增一行状态（运行中/已停止 + **运行中的**间隔 + 上次成功检查 + 该轮新作品数）。刻意返回**副本**而非把 `alive` 塞进 `_auto_follow_state`（该 dict 由自动关注线程与 `/api/auto-follow/config` 共用，派生值会污染运行态）。界面文案按真实语义措辞：`last_check` 只在**成功拉到关注列表并处理完一轮**时更新（拉不到任何作品的那轮直接 `continue`），所以"陈旧"既可能是没有新作品、也可能是每轮都在失败 —— 不能写成"上次轮询时间"。**证伪**：① 去掉 `alive` → 4 例失败（`KeyError: 'alive'` ×3 + 运行态污染用例）；② 把 `alive` 写死成 `True` → 两个"线程已停止"用例失败于 `assert True is False`（证明它读的是真实线程引用而非常量）；③ 删掉页面加载时的调用 → 前端接线用例失败。**遗留**：仍无 supervisor / 自动重启（§33.1 第 4 条不变）；自动关注**没有** `last_error`（出错只写日志、不写 state），所以"刚失败过"与"关注列表本来没新作品"在界面上仍不可区分 —— 该遗留已在 **S22** 处理（见下）。
3. **S21 设置页写 Cookie 不是原子写**（`routes_settings.api_settings_post` → 新增 `helpers._atomic_write_text`）。S19 换对了落点，但写盘仍是 `open(path, 'w')`：它**先截断再写**，而读侧 `fetcher._load_cookie()` 在**其它线程**读同一路径（生产 `gunicorn -w 1 --threads 8`）。读到空串时它会把 `_cookie_value` 置空**并连同 mtime 一起缓存**（`if mtime != _cookie_mtime` 之外没有任何二次校验），于是那条线程/那个连接池会一直用空 Cookie，**直到文件 mtime 再变**——症状是"设置页提示保存成功、搜索仍 401/空结果，重启才恢复"。这条在 S19 证伪过程中被顺带复现（夹具把仓库根真实 Cookie 写坏），当时只做记录未修。
   修法：新增 `helpers._atomic_write_text`（同目录 `*.tmp` → `flush` + `fsync` → `os.replace`，失败清理 tmp，异常原样抛出），Cookie 分支改用它；`_atomic_write_json`（settings.json，S16）**刻意不动**，避免改动已稳定且有用例覆盖的行为。三处配套语义：① **保留目标文件原有权限位**（`open(...,'w')` 对已存在文件是保留原模式的，而 tmp 的模式来自 umask，直接 replace 会把加固过的 0600 悄悄放宽成 0644）；② **不创建父目录**（Cookie 落点由 `config.COOKIE_PATH` 决定，目录不存在属于部署错误，静默 makedirs 会把错误藏起来）；③ `os.replace` 对 **PermissionError 有界重试**（5 次、退避 20→100ms）——Windows 上读侧正持着目标文件时 replace 会抛 `WinError 5`，这是实测出来的（POSIX 的 rename 无此问题，生产 Linux 不受影响，但不重试就会把开发机/Windows 部署的"保存 Cookie"变成偶发 500）。
   **证伪**：三条新用例先在旧实现上跑，全部失败且症状与预判一致 —— `test_cookie_write_is_an_atomic_swap` 观测不到任何 tmp+replace；`test_failed_cookie_write_keeps_the_previous_cookie` 变成 `assert 200 == 500`（旧实现"保存失败"却把旧 Cookie 弄成空文件）；`test_concurrent_reader_never_sees_a_truncated_cookie` **实测读到空串**（120 轮写入期间 12940 次读里 5 次为空；把读侧节奏放宽到 2ms 轮询后仍能复现，575 次读里多次为空）。修完再跑：530 例 = 524 passed / 2 skipped / 4 failed（那 4 例是预先存在的 Windows 沙箱子进程检查）。**部署语义变化**：写了 tmp 就必须能创建/改名，因此**目录可写**成为新要求（只给文件写权限的 `/etc` 部署会 500）——已写进 `AGENTS.md` 与 `docs/maintenance.md`。**边界**：POSIX 专属断言（权限位保留、rename 对读者零影响）在 Windows 开发机上无法验证，只能 skip；`_load_cookie` 的读侧没有对 0 字节文件的二次校验（mtime 变了但内容为空时仍会缓存空值）——修在写侧，读侧留给将来。
4. **S22 自动关注"最近一轮有没有出错"看不见**（`background._auto_follow_worker` → `runtime._auto_follow_state['last_error']`）。S20 把"线程存活"接上了界面，但**没有** `last_error`：worker 的 `except Exception` 只写一行日志、不落 state。而 `last_check` / `last_count` 只在**成功跑完一轮**时更新，于是"上次成功检查很旧"既可能是"关注列表里本来没有新作品"、也可能是"每一轮都在失败" —— 界面上分不开，而这正是 S20 想要解决的静默失效。
   修法：`runtime._auto_follow_state` 增补 `last_error`（写在**初始字面量**里 —— 该 dict 的并发约定是"键集合固定、只改值"，懒加键会让 `jsonify` 抛 `dictionary changed size during iteration`），worker 在 `except` 里写、在**干净收尾处**清；`/api/auto-follow/status` 因为返回 `dict(_auto_follow_state)` 自动带上它，无需改路由；设置页状态行加一句"最近一轮出错：…"并把该行标红（线程活着但每轮都失败也要标红 —— 那才是这个字段的意义所在）。**两条刻意不做的**：① 拉不到任何作品**不**写 `last_error`（Cookie 失效时 Pixiv 静默返回空结果，写进去就是假告警）；② 那种轮次也**不**清旧错误（会抹掉真证据）。所以"空列表"这层含糊依然存在，改由界面文案如实说明。
