...
### 33.4 S18 之后的三个跟进项（S19 / S20 / S21）

这三条是**收口阶段之后**才处理完的：S19 来自本文档之外的分析文档发现（`docs/technical-documentation.md` §20.2），S20 是 §16 那条"状态 API 主动告警"建议的落地补完，S21 是 S19 自己写下的遗留项（Cookie 写入非原子）。三者都不改既有语义分支，改动范围分别在"一个路径来源"、"一个响应字段 + 一行界面文案"、"一个写盘原语"。
