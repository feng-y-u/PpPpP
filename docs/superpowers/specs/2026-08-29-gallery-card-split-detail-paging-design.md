...
- 容量控制：图库侧只写当前页；详情页拉取相邻页时回写并只保留目标页 ± 1（见 §2 `fetchPage`）
- `v` 为结构版本号，旧结构数据直接丢弃
...
**发现的边界（已决策处理）**：本实例 5 个作品中 4 个是孤儿作品（本地有文件但 `Illust` 表无记录），翻页/信息区点击落到这些作品会 404（§3 既定行为）。后续决策：删除接口已支持孤儿（`6639a7e`：无 DB 行时按 `downloads/<pid>` 目录删 + 记 DownloadLog，批量删除同样覆盖，孤儿删除后重置 `_scan_cache`），图库孤儿卡片可直接删除；清理孤儿后 404 场景随之消失，无需再做"翻页跳过孤儿"。
