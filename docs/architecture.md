...
| `fetcher.py` | Pixiv API 封装 | Cookie/OAuth 认证、`search_by_tag`/`search_by_user`/`browse_discovery`/`paginated_search`/`fetch_following`、作品详情、`clear_search_cache`、`get_detail_error_samples`、`_insert_new_illusts`（冲突容忍入库） |
